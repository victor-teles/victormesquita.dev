...
Antes de comerçar, o Drizzle ORM (ou somente Drizzle) é um Object Relational Mapper (ORM) leve, rápido e *typesafe*.
Ele permite que você escreva suas queries no formato similar ao SQL e oferece suporte nativo a serverless.
...
Isso significa que você pode encontrar suporte, tutoriais e exemplos que serão de grande ajuda na hora de aprender e resolver problemas.
...
<Step title="Configurar a conexão com o banco">
...
```typescript
const userWithPosts = await db._query.users.findMany({
	with: {
		posts: true			
	},
});
