...
const Folder: React.FC<FolderProps> = ({ name, children, open, note }) => {
  ...
  return (
    <li role="treeitem" aria-expanded={isOpen} tabIndex={0} onKeyDown={handleKeyDown} aria-selected={isFocused}>
      <a
        onClick={handleClick}
        onFocus={handleFocus}
        onBlur={handleBlur}
      ...
      >
        <div className={styles.folder}>
          ...
          <span>
            {name}
            <span className="sr-only">, {isOpen ? 'open' : 'closed'} folder</span>
...
const File: React.FC<FileProps> = ({ type, name, note, url: link }) => {
  ...
  return (
    <li role="treeitem" aria-selected={isFocused} onFocus={handleFocus} onBlur={handleBlur}>
