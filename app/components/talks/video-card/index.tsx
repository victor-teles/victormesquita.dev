...
const VideoCard = ({
  talk: { title, description, date, id, thumbnail, myDescription, channel, views, likes, tags, lengthSeconds },
}: {
  talk: Talk;
}) => {
  const EmbedCode = () => {
    ...
    if (isClicked) {
      return (
        // render as HTML
        <div className={styles.embed} dangerouslySetInnerHTML={{ __html: embedCode }} />
      );
    }
    return (
