...
const ControlPanel: FC<Props> = ({ minWidth = 940 }) => {
  const { setIsTimerActive, isTimerActive, time, setTime, intervalId, selectedItems } = useBenchmarkContext();
    ...
    <div className={styles.content}>
      <div className={styles.control}>
        <div className={styles.time}>
          ...
          {!isTimerActive && !isConfigOpen && (time === maxDataLength * 100 || time === 0) && (
            <>
              <div className={styles.divider}></div>
              <button type="button" className={styles['play-wrap']} onClick={rerun}>
                <svg
          ...
          {!isTimerActive && !isConfigOpen && time !== maxDataLength * 100 && time !== 0 && (
            <>
              <div className={styles.divider}></div>
              <button type="button" className={styles['play-wrap']} onClick={resume}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
          ...
          {isTimerActive && !isConfigOpen && (
            <>
              <div className={styles.divider}></div>
              <button type="button" className={styles['play-wrap']} onClick={pause}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
          ...
          {!isConfigOpen && <Rewind maxDataLength={maxDataLength} time={time} rewind={rewind} />}
        </div>
        <div className={styles.config}>
          ...
          <button type="button" className={styles['config-button']} onClick={toggleConfigModal}>
            <OptionsIcon />
          </button>
        </div>
      </div>
        ...
        <div className={isBlurred ? styles.blurred : styles['hide-blur']}>
          {drizzleData && compareData && (
            <div className={styles['blur-content']}>
              ...
              {isSmall && (
                <button onClick={start} type="button" className={isShaking ? styles['start-shaked'] : styles.start}>
